...
public class TicTacToeGUI extends JFrame {
    ...
    public TicTacToeGUI() {
        ...
        setLocationRelativeTo(null); // Das Fenster öffnet sich genau in der Mitte
    ...
    private void buttonGeklickt(int z, int s) {
        ...
        if (zuege == 9) {
            ...
            aktualisiereAnzeige();
            JOptionPane.showMessageDialog(this, "Unentschieden! Es gibt kein Gewinner");
            spielZuruecksetzen();
            return;
        }
    ...
    private void starteGewinnAnimation(int[][] felder) {
        ...
        final int[] wiederholungen = {0};

        timer.addActionListener(e -> {
            // Blink-Logik
            Color c = (wiederholungen[0] % 2 == 0) ? Color.BLACK : null;
            if (felder != null) {
                for (int[] f : felder) {
                    buttons[f[0]][f[1]].setBackground(c);
                }
            }
            wiederholungen[0]++;

            // Wenn das Blinken fertig ist (nach 6 Mal)
            if (wiederholungen[0] > 6) {
                timer.stop();

                // HIER kommt die Meldung zurück:
                String gewinnerName = (aktuellerSpieler == symbolSpieler1) ? name1 : name2;
                JOptionPane.showMessageDialog(this, "Herzlichen Glückwunsch! " + gewinnerName + " hat gewonnen!");
